...
niftis_directory = "/DATA/ECLIPSE_niftis/"
...
for count, ind in enumerate(individuals):
  ...
  scans = glob.glob(dicoms_directory + "/" + ind + "/*STD*[!mask]*L2*.tar.gz")
  if len(scans) == 0:
    print("Individual " + ind + " does not have STD/L1 images")
    continue
  ...
  if is_control:
    # Decompress scan
    try:
      tar = tarfile.open(scans[0], "r:gz") 
      tar.extractall(dicoms_directory + '/' + ind)
      tar.close()
      # Convert scan and place in aux directory
      dicom2nifti.convert_directory(scans[0][:-7], niftis_directory + 'aux')
      # Rename file from aux directory
      os.rename(niftis_directory + 'aux/' + os.listdir(niftis_directory + 'aux')[0], niftis_directory + 'aux/' + os.path.basename(scans[0])[:-7] + '.nii.gz')
      # Move file from aux directory and move to case directory
      shutil.move(niftis_directory + 'aux/' + os.listdir(niftis_directory + 'aux')[0], niftis_directory + 'control')
    except:
      print("An exception occurred while processing file: " + scans[0])
  ...
  # Case handle
    ...
    try:
      ...
      os.rename(niftis_directory + 'aux/' + os.listdir(niftis_directory + 'aux')[0], niftis_directory + 'aux/' + os.path.basename(scans[0])[:-7] + '.nii.gz')
      # Move file from aux directory and move to case directory
      shutil.move(niftis_directory + 'aux/' + os.listdir(niftis_directory + 'aux')[0], niftis_directory + 'case')
    except:
      print("An exception occurred while processing file: " + scans[0])
      shutil.rmtree(niftis_directory + "aux")
      os.makedirs(niftis_directory + "aux")
