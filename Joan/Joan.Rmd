...
```{python, results='hide', warnings=FALSE, message=FALSE}
def load_scan(path):
...
```{python, results='hide', warnings=FALSE, message=FALSE, cache=TRUE}
...
```{r, echo=FALSE, fig.cap = "Some HU values.", label="HU_values", out.width="40%", out.height=="40%", fig.align="center", cache=TRUE}
knitr::include_graphics("./Images/HU_values.png")
```
...
```{python, results='hide', warnings=FALSE, message=FALSE, collapse=TRUE, cache=TRUE}
...
We now have enough information to plot the DICOM image in 3D space. We are doing it by creating a high-quality static using 3D capability of `matplotlib` (`plt_3d` function) and a interactive render using `plotly`, which has WebGL support via JavaScript (`plotly_3d` function). The [marching cubes](https://en.wikipedia.org/wiki/Marching_cubes) algorithm is used to generate a 3D mesh from the dataset.
...
```{python, error=FALSE, warnings=FALSE}
import random
from scipy import ndimage
...
```{python, error=FALSE, warnings=FALSE, eval=FALSE, results=FALSE}
