...
We select train 70% and test 30% randomly:
```{r}
rows <- sample(nrow(df_rdr4), .7 * nrow(df_rdr4))
train_data <- df_rdr4[rows, ]
test_data <- df_rdr4[-rows, ]
```


Now we will use Random Forest with caret and balancing data:
```{r}
library(caret)
# 10-fold cross-validation using downsampling
...
set.seed(42)
model_rf_under <- caret::train(TRTGRP_values ~ ., 
                               data = train_data,
                               method = "rf",
                               trControl = ctrl)
model_rf_under
...
Prediction:
```{r}
pred_rf <- predict(model_rf_under, newdata = test_data, type = "raw") 
oj_cm_rf <- confusionMatrix(pred_rf, test_data$TRTGRP_values) 
oj_cm_rf
```
...
```{r}
100 * sum(diag(confusion_matrix)) / sum(confusion_matrix)
```

<span style="color:red">Better results without balancing data?</span>
