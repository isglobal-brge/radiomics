...
We will make an introduction to **Chronic obstructive pulmonary disease (COPD)**.\
...
print(as_tibble(clinic_data_clean)) # view firsts values
```
...
print(as_tibble(df_rdr))
```

Load the clean data set:
```{r}
clean_data <- read.delim("eclipse_all_CLEAN.txt")
print(as_tibble(clean_data)) # view firsts values
```
