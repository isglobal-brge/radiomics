...
Now we will display three images.

```{r, out.width = "300px"}
image1 <- readDICOMFile("/Users/andrealetaalfonso/Desktop/TFG/images/1.dcm")
image(t(image1$img), col=grey(0:64/64), axes=FALSE, xlab="", ylab="")

image2 <- readDICOMFile("/Users/andrealetaalfonso/Desktop/TFG/images/2.dcm")
image(t(image2$img), col=grey(0:64/64), axes=FALSE, xlab="", ylab="")

image3 <- readDICOMFile("/Users/andrealetaalfonso/Desktop/TFG/images/3.dcm")
image(t(image3$img), col=grey(0:64/64), axes=FALSE, xlab="", ylab="")
```

### Radiomic features
To compute the radiomic features of the images using RIA, we first need to comvert the dicom images to RIA_image class. To do this, we will use the function load_dicom. We will analyze 10 images.

```{r, results = FALSE, message=FALSE}
DICOM <- RIA::load_dicom(filename="/Users/andrealetaalfonso/Desktop/TFG/images/", skipFirst128=FALSE, DICM=FALSE, boffset = 128)
```

To compute the first-order statistics we use first_order.

```{r, message=FALSE}
DICOM = first_order(RIA_data_in = DICOM)
first_order_data <- RIA:::list_to_df(DICOM$stat_fo$orig)
first_order_data
```
