...
We can check that we exported the file correclty with: *read.delim("radiomic_features.txt")*.
...
Or, if we want, we can combine both results into one *.txt* file.
```{r}
all_results <- rbind.data.frame(results, results2)
write.table(results2, file="radiomic_features.txt", sep="\t")
```
...
```{r}
rdr <- import_radiomic_table("/Users/andrealetaalfonso/Desktop/TFG/radiomic_features.txt")
rdr
```
