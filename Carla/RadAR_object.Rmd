...
identical(id, substr(unlist(lapply(sapply(colData(rdr)$mask_id, strsplit, "_"), "[[", 1)), 2, 12))

# Check the order of time
identical(time, unlist(lapply(sapply(colData(rdr)$mask_id, strsplit, "_"), "[[", 4)))
```
...
rownames(clinical_change) <- clinical_test$id
identical(colnames(rdr_L1), rownames(clinical_change))
...
colData(rdr_L1) <- cbind(colData(rdr_L1), clinical_filtered)
